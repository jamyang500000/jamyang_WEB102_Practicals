...
exports.createVideo = async (req, res) => {
  try {
    ...
    const {
      caption,
      audioName,
      // Practical 5 direct-upload flow: frontend already uploaded to Supabase
      videoUrl: providedVideoUrl,
      thumbnailUrl: providedThumbnailUrl,
      videoStoragePath: providedVideoPath,
      thumbnailStoragePath: providedThumbnailPath,
    } = req.body;

    // ─── Path A: metadata-only (frontend uploaded directly to Supabase) ───
    if (providedVideoUrl) {
      const newVideo = await prisma.video.create({
        data: {
          ...
          videoUrl: providedVideoUrl,
          thumbnailUrl: providedThumbnailUrl || null,
          videoStoragePath: providedVideoPath || null,
          thumbnailStoragePath: providedThumbnailPath || null,
        },
        include: {
    ...
    // ─── Path B: legacy multipart upload (file passes through backend) ───
...
// Add a comment to a video (Practical 4 - frontend posts to /api/videos/:id/comments)
exports.addCommentToVideo = async (req, res) => {
  try {
    ...
    console.error(`Error adding comment to video ${req.params.id}:`, error);
